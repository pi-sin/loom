...
import io.loom.example.builder.AssembleProductBuilder;
import io.loom.example.builder.FetchPricingBuilder;
import io.loom.example.builder.FetchProductBuilder;
import io.loom.example.builder.FetchRecommendationsBuilder;
import io.loom.example.builder.FetchReviewsBuilder;
...
@LoomApi(method = "GET",
         ...
         tags = {"Products"},
         queryParams = {@LoomQueryParam(name = "currency", description = "Price currency (e.g. USD, EUR)"),
                 @LoomQueryParam(name = "fields", description = "Comma-separated fields to include in the response")},
         headers = {@LoomHeaderParam(name = "X-API-Key", required = true, description = "API authentication key")},
...
@LoomGraph({@Node(builder = FetchProductBuilder.class, timeoutMs = 3000),
        ...
        @Node(builder = FetchRecommendationsBuilder.class,
              required = false,
              timeoutMs = 2000,
              dependsOn = {FetchProductBuilder.class, FetchPricingBuilder.class}),
        @Node(builder = AssembleProductBuilder.class,
              dependsOn = {FetchProductBuilder.class, FetchPricingBuilder.class, FetchReviewsBuilder.class,
                      FetchRecommendationsBuilder.class})})
